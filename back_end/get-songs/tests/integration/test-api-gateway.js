...
describe("Test API Gateway", function () {
   ...
   * here we use cloudformation API to find out what the GetSongsApi URL is
   */
  before(async () => {
    ...
    const apiOutput = stackOutputs.find(
      (output) => output.OutputKey === "GetSongsApi"
    );

    expect(apiOutput, `Cannot find output GetSongsApi in stack ${stackName}`)
      .not.to.be.undefined;
